...
Персистентный каталог единиц-сервисов в PostgreSQL: схема и миграции, repository с guarded-CAS-переходами статусов и транзакциями, keyset-пагинация, usecase-слой и доменная реализация gRPC-чтения (`GetService`/`ListServices`/`CreateServiceRecord`), content-aware `/readyz` с пингом Postgres.
## Requirements
