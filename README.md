...
**Локально** `demo-user` засеян ролью `iam-admin` с правами
`(read|write|manage, iam:global)` (миграции `0006`/`0008`), поэтому раздел
работает при включённом RBAC. Проверка отказа: снять у `demo-user` нужный грант →
раздел/форма отвечает `403` и скрывает соответствующие действия.
