...
// ролями на DataTable с КУРСОРНОЙ пагинацией (next_page_token, ADR-0009). На экране —
// только таблица; назначение роли вынесено в МОДАЛКУ (Dialog), открываемую кнопкой
// «Назначить роль» в PageHeader.actions (паттерн «таблица + кнопка → модалка», как на
// «Ролях»/«Правах»). Назначение — ТОЛЬКО выбором пользователя из справочника Keycloak
// через пикер с поиском (debounce, username/email, канонический subject). Снятие роли —
// ConfirmDialog. «Осиротевшие» субъекты (found=false) помечаются «нет в каталоге».
// 403 на админку — fail-closed (IamGuard); 403 на iam:directory — пикер скрыт с
// отказом; 503 каталога — индикация, поиск остаётся. Ответы валидируются zod в общих
// хуках.
import { useState } from "react";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { AlertTriangle, Loader2, Plus, Search, ShieldX, Trash2 } from "lucide-react";
import { z } from "zod";
...
export function UsersPage() {
  ...
  // resetAssign — единый сброс состояния модалки (форма + пикер + поиск). Зовётся
  // и после успешного назначения, и при закрытии модалки, чтобы повторное открытие
  // было чистым.
  const resetAssign = () => {
    reset();
    setPicked(null);
  ...
  // closeAssign — закрыть модалку со сбросом состояния.
  const closeAssign = () => {
    setAssignOpen(false);
    resetAssign();
  };
  ...
  return (
    <section className="flex flex-col gap-5">
      ...
      <IamGuard error={subjectsQuery.error}>
        {/* На экране — только таблица субъектов с их ролями. */}
        <DataTable
      ...
      {/* Модалка назначения роли: пикер из справочника + выбор роли. */}
      <Dialog
        open={assignOpen}
        onClose={closeAssign}
        title="Назначить роль пользователю"
        description="Выдача существующей роли пользователю из каталога. Найдите пользователя и выберите его. Операция идемпотентна."
        footer={
          <>
            ...
            <Button
              ...
              disabled={assignMutation.isPending || directoryForbidden}
            >
